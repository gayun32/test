...
- 콘솔에서 메뉴 번호를 입력해 기능을 선택하는 프로그램입니다.
- 프롬프트 추가, 목록 조회, 카테고리별 조회, 검색, 상세 보기,
  즐겨찾기 기능을 제공합니다.
- 데이터는 프로그램 실행 중에만 유지되며, 종료 시 초기화됩니다.
"""
...
def show_detail(prompts):
    ...
    star = "⭐ 즐겨찾기 중" if prompt["favorite"] else "즐겨찾기 아님"
...
def toggle_favorite(prompts):
    ...
    if prompt["favorite"]:
        print(f"\n  ⭐ '{prompt['title']}'을(를) 즐겨찾기에 담았어요!")
    else:
        print(f"\n  '{prompt['title']}'을(를) 즐겨찾기에서 뺐어요.")
...
def show_favorites(prompts):
    ...
    for i, prompt in enumerate(favorites, start=1):
        print(format_prompt_line(i, prompt))
    print(f"  총 {len(favorites)}개의 즐겨찾기가 있어요.")
